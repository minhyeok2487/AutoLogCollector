...
// ExecuteCommandsWithLog connects to server and executes commands with real-time log callback
func ExecuteCommandsWithLog(server Server, creds *Credentials, commands []string, onLog func(line string)) (string, error) {
	// SSH config with legacy algorithm support for older Cisco devices
	config := &ssh.ClientConfig{
		User: creds.User,
		Auth: []ssh.AuthMethod{
